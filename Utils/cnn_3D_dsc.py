...
class ResnetBuilder(object):
    ...
    def build(input_shape, num_outputs):
        ...
        pool1 = AveragePooling3D(pool_size=(2, 2, 2), strides=(1, 1, 1), padding='same')(act1)

        conv2 = SeparableConvolution3D(filters=192, kernel_size=(2, 2, 2), strides=(1, 1, 2),
                       kernel_regularizer=regularizers.l2(0.01),data_format = 'channels_last')(pool1)
...
def main():
    model = ResnetBuilder.build_resnet_8((1, 7, 7, 108), 16)
    model.compile(loss="categorical_crossentropy", optimizer="sgd")
    model.summary()#显示训练过程中的数据尺寸和训练参数变化
